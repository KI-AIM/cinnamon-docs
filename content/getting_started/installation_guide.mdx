...
Download the [docker-compose.yml](https://github.com/KI-AIM/Cinnamon/blob/main/docker-compose.yml) and [.env](https://github.com/KI-AIM/Cinnamon/blob/main/.env) from Cinnamon's repository.
Set the password of the database user in the downloaded .env file.
...
1. Set the URLs for the client

The client needs to be able to reach all modules.
The Cinnamon Platform must run at the root of the server, otherwise the website will not be able to fetch the required resources.
To configure the URLs of other modules, use the `CINNAMON.EXTERNAL-SERVER.<X>.URL-CLIENT` variable.
...
services:
    cinnamon-platform:
        environment:
            CINNAMON.EXTERNAL-SERVER.0.URL-CLIENT: # URL to Cinnamon Anonymization
            CINNAMON.EXTERNAL-SERVER.1.URL-CLIENT: # URL to Cinnamon Synthetization
            CINNAMON.EXTERNAL-SERVER.2.URL-CLIENT: # URL to Cinnamon Evaluation
            CINNAMON.EXTERNAL-SERVER.3.URL-CLIENT: # URL to Cinnamon Risk-Assessment
```
...
3. (Optional) Configure demo server
...
4. (Optional) Configure the workflow

In case you want change the default workflow or use your own algorithms see the [Configuring Cinnamon](/configuration_customization/configuration_properties) section for advanced configurations.
